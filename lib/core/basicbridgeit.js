 ...
 *
 * This adapter is a very basic HTTP adapter for specifically making calls to BridgeIt
...
BasicBridgeItAdapter.prototype.validateConfig = function (config) {
    ...
    if (!config.host) {
        log('missing host', config);
        return false;
    }

    if (!config.service) {
        log('missing service', config);
        return false;
    }

    if (!config.account) {
        log('missing account', config);
        return false;
    }

    if (!config.realm) {
        log('missing realm', config);
        return false;
    }

    if (!config.path) {
        log('missing path', config);
        return false;
    }
    ...
    serviceRequest.end(function (res) {
        ...
        if (ld.isEmpty(payload) && res.text) {
            try {
                ...
                console.error('problem parsing response text', payload, res.text, e);
                cb(e);
                return;
            }
        }
